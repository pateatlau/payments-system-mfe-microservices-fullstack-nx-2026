/**
 * Admin MFE Rspack Configuration
 *
 * Module Federation v2 Remote - exposes admin dashboard components
 *
 * Exposes:
...
function resolvePublicPath(isProduction) {
  ...
  if (isProduction && !raw) {
    ...
    throw new Error(
      '[admin-mfe] NX_PUBLIC_PATH is required in production.\n' +
        'Set it in Vercel project env vars, e.g.: https://admin-mfe-theta.vercel.app/'
    );
  }
  if (!raw) return 'auto';
...
function resolvePaymentsMfeRemote(isProduction) {
  ...
  if (!base) {
    if (process.env.CI === 'true') {
      ...
      return 'paymentsMfe@https://placeholder.vercel.app/remoteEntry.js';
    }
    throw new Error(
      '[admin-mfe] NX_PAYMENTS_MFE_URL is required in production.\n' +
        'Set it in Vercel project env vars, e.g.: https://payments-mfe.vercel.app'
    );
  }
  const url = base.endsWith('/remoteEntry.js')
    ? base
    : `${base}/remoteEntry.js`;
  return `paymentsMfe@${url}`;
}
...
module.exports = (env, argv) => {
  ...
  const paymentsMfeRemote = resolvePaymentsMfeRemote(isProduction);
  return {
    ...
    entry: './src/main.tsx',
    output: {
      path: path.resolve(__dirname, '../../dist/apps/admin-mfe'),
      ...
      // (e.g. https://admin-mfe-theta.vercel.app/) set in the Vercel project env vars.
      publicPath,
