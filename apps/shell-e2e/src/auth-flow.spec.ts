...
test.describe('Authentication Flow', () => {
  ...
  test('should complete sign-in flow: sign in → redirect → payments page', async ({ page }) => {
  ...
  test('should complete sign-up flow: sign up → redirect → payments page', async ({ page }) => {
  ...
  test('should show validation errors for weak password', async ({ page }) => {
    ...
    // Should show validation error
    await expect(page.locator('text=/password.*(?:strength|length|complexity)/i')).toBeVisible({ timeout: 5000 });
  });
  ...
  test('should navigate between sign-in and sign-up pages', async ({ page }) => {
    ...
    const signUpLink = page.locator('a, button').filter({ hasText: /sign up|signup/i }).first();
    ...
    const signInLink = page.locator('a, button').filter({ hasText: /sign in|signin/i }).first();
